...
from netAct_sql_reader_pairs import AtomTypes
...
class Table:
    ...
    # saulo
    '''
    Builds files for NetAct genes
    '''
    def _build_netact(self):

        # this is to build the groups in which the NetAct genes take part
        # so translates the full table data to MeTTa
        if "_group" in self.name:
            return
        string_list = ["Su(var)205", "Top3beta", "Mef2", "Clk", "Dref", "TfIIB", "Myc", "AGO2", "Nipped-B", "Cp190",
                       "TfIIA-L",
        ...
        df = pandas.DataFrame(relevant_lines, columns=df.columns)
...
class Genotype_phenotype_data_table(Table):
    ...
    in which the first element in the very column value.
    '''
