...
#SQL_FILE = "/mnt/hdd_2/saulo/snet/hyperon/das/data/flybase/input/2023_05/FB2023_05.sql"
...
#OUTPUT_DIR = "/mnt/hdd_2/saulo/snet/hyperon/das/data/flybase/output/2023_05/metta"
...
class LazyParser():
    ...
    def __init__(self, sql_file_name, release_id, precomputed=None):
    ...
    def _read_table_pairs(self):
        table_pairs_dict = {}
        with open(f"{PRECOMPUTED_DIR}/essential_pairs_{self.release_id}.txt", 'r') as file:
            ...
            for row in reader:
                ...
                for i in range(0, len(column_names) - 1, 2):
                    if column_names[i] != '' and column_names[i + 1] != '':
                        pairs.append((column_names[i].strip(), column_names[i + 1].strip()))
                        pairs.append((column_names[i + 1].strip(), column_names[i].strip()))
                table_pairs_dict[table_name] = pairs
        return table_pairs_dict

    def _emit_precomputed_tables(self):
        table_count = 0
        pairs_dict = self._read_table_pairs()
        for table in self.precomputed.all_tables:
            if SHOW_PROGRESS:
                self._print_progress_bar(table_count, len(self.precomputed.all_tables), 50, 3, 5)
            # list of tuples: each tuple in the list is a pair of column names
            pairs = pairs_dict[_clear_table_name(table.name)]
            print(f"Starting table:::-------------------> {_clear_table_name(table.name)}")
            for row in table.rows:
                for column_pair in pairs:       # print(row)
                    for key1, value1 in zip(table.header, row):  # handles mapped columns of precomputed tables
                        if key1 != column_pair[0]:
                            continue
                        ...
                        for key2, value2 in zip(table.header, row):
                            ...
                            if key1 == "FB_group_id" and key2 == "Parent_FB_group_id":
                                ...
                                continue
                            if key2 == "FB_group_id" and key1 == "Parent_FB_group_id":
                                continue
            ...
            print(f"Table:::-----------------------------> {_clear_table_name(table.name)}  finished!")
            table_count += 1
            if SHOW_PROGRESS:
                self._print_progress_bar(table_count, len(self.precomputed.all_tables), 50, 3, 5)
    ...
    def _start_copy(self, line):
        ...
        # saulo update to python 3.12 raised an error at (below) line
        # schema_columns = [column['name'] for column in self.table_schema[self.current_table]['columns']]
        #if not self.current_table.startswith("public"):
         #   print(f'self.current_table  :::------------> {self.current_table}')
          #  return False
    ...
    def _foreign_key(self, first_line, second_line):
        try:
            ...
            line = second_line.split()
            field = line[5][1:-1]
            # saulo: only "public" tables are available for querying (2023_04 forward)
            if table.startswith("public."):
...
def main():
    release_id = "2023_05"
    precomputed = PrecomputedTables(PRECOMPUTED_DIR, release_id) if PRECOMPUTED_DIR else None
    parser = LazyParser(SQL_FILE, release_id, precomputed)
    parser.parse()
